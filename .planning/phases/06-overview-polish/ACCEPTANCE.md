# AgentMind v1.0 Acceptance (SPEC §10)

Recorded: 2026-08-01  
Build: `npm run tauri build` → `src-tauri/target/release/bundle/macos/AgentMind.app`  
...
| 5 | 五个主视图无死链；Cmd+K 可跳转；macOS `.app` 可启动 | **Pass** | overview / agents / agent-detail / commander / tasks; Cmd+K expanded (06-02); `AgentMind.app` bundled and contains `Contents/MacOS/agentmind`. |
...
- `npm run tauri build` → `AgentMind.app` present
...
- Bundle identifier remains `com.agentmind.app` (Tauri warns `.app` suffix); functional, not renamed mid-v1 to avoid churn.
